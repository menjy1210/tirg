...
class Fashion200k(BaseDataset):
  ...
  def __init__(self, path, split='train', transform=None):
    ...
    for filename in label_files:
      ...
      # example: 
      # women/dresses/casual_and_day_dresses/56037632/56037632_0.jpeg	-5.070000	black knit midi dress
      print('read: ' + filename)
      ...
      for line in lines:
        # line has three parts: image path, detection_score[?], text[?caption]
        line = line.split('	') 
  ...
  def generate_test_queries_(self):
    ...
    '''
    read test queries from the file

    output:
    lines: list of strings, all the lines in the file
    '''
    with open(self.img_path + '/test_queries.txt') as f:
      lines = f.readlines()

    '''
    for test, get the source and target image path
    '''
    self.test_queries = []
    for line in lines:
      ...
      self.test_queries += [{
          ...
          'mod': {
              'str': mod_str # ='replace ' + source_word + ' with ' + target_word
          }
      }]
